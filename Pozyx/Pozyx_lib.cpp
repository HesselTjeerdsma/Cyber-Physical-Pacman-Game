...
int PozyxClass::setPositionFilter(int filter_type, int filter_strength, uint16_t remote_id)
{
	assert((filter_type == POZYX_POS_FILTER_NONE) || (filter_type == POZYX_POS_FILTER_FIR) || (filter_type == POZYX_POS_FILTER_MOVING_AVERAGE) || (filter_type == POZYX_POS_FILTER_MOVING_MEDIAN));
	assert((filter_strength >= 0) && (filter_strength <= 15));

	int status;
	uint8_t params = filter_type + (filter_strength << 4);

	if (remote_id == NULL) {
		status = regWrite(POZYX_POS_FILTER, &params, 1);
		delay(POZYX_DELAY_LOCAL_WRITE);
	}
	else {
		status = remoteRegWrite(remote_id, POZYX_POS_FILTER, &params, 1);
		delay(POZYX_DELAY_REMOTE_WRITE);
	}
	return status;
}
...
int PozyxClass::getEulerHeading_deg(float32_t *euler_heading, uint16_t remote_id)
{
	assert(euler_heading != NULL);

	//raw data is one 16bit value
	int16_t raw_data = 0;
	int status;

	if(remote_id == NULL){
		status = regRead(POZYX_EUL_HEADING, (uint8_t *)&raw_data, sizeof(int16_t));
	}
	else {
		status = remoteRegRead(remote_id, POZYX_EUL_HEADING, (uint8_t *)&raw_data, sizeof(int16_t));
	}

	*euler_heading = raw_data / POZYX_EULER_DIV_DEG;
