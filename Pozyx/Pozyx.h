 ...
 */
    ...
    */
	...
	/**
	* Configure filter used by the positioning algorithm.
	* This function configures the positioning filter and the physical dimension by writing to
	* register reg: POZYX_POS_FILTER
	*
	*	@param algorithm used to filter the data. Possible values are #POZYX_POS_FILTER_NONE, #POZYX_POS_FILTER_FIR, #POZYX_POS_FILTER_MOVING_AVERAGE or #POZYX_POSFILTER_MOVING_MEDIAN
	*	@param the strength of the filter (number is equal to points used). Possible values 0-15
	*	@param remote_id optional parameter that determines the remote device to be used
	*
	* @retval #POZYX_SUCCES succes
	* @retval #POZYX_FAILURE function failed
	*/
	static int setPositionFilter(int filter_type = POZYX_POS_FILTER_NONE, int filter_strength = 0x0, uint16_t remote_id = NULL);
 ...
 */
    ...
    */
	...
	/**
	* Obtain the heading in Euler angles in degrees
	* This function reads out the Euler angle: Yaw that is part of the 3D orientation of the device
	*
	*	@param euler_heading: reference to the heading float.
	*	@param remote_id: optional parameter that determines the remote device to be used
	*
	* @retval #POZYX_SUCCESS success.
	* @retval #POZYX_FAILURE function failed.
	* @retval #POZYX_TIMEOUT function timed out, no response received.
	*/
	static int getEulerHeading_deg(float32_t *euler_heading, uint16_t remote_id= NULL);
